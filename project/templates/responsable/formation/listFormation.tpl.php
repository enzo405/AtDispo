<div class="contenuPageAValider">
    <div class="contenuListeAValider grid-5">
        ...
        <?php foreach ($formationsList as $formations) {
                foreach ($formations as $formation) { ?>
                    <p class="<?= ($formation->nomFormation->valide == 1 ? '' : 'standby-validation')?>"><?= $formation->toString() ?></p>
                    ...
                    <p>
                        <span class="link-button"><a href="<?= SITE_ROOT ?>/responsable/formations/<?= $formation->id ?>">Voir la formation</a></span>
                    </p>
                <?php }
            }
        } else {
    ...
    <div class="pagination">
        <?php for ($i = 1; $i <= $numberOfPages; $i++) { ?>
            <a href='/responsable/formations?page=<?= $i ?>'> <?= $i ?> </a>
        <?php } ?>
    </div>
</div>